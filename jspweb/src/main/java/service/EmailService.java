...
public class EmailService { // 이메일 전송에 관련된 클래스 
	...
	public boolean authsend( String toEmail, String contentHTML) {
		...
		// 1. 호스팅 설정 [ 네이버 기준 ]
		Properties Properties = new Properties(); // Properties : 키와 값으로 구성된 map 컬렉션 중하나.
		...
		Authenticator authenticator = new Authenticator() {
			...
			// Authenticator :import javax.mail.Authenticator;
				// = 패스워드 검증[인증] 메소드 구현
				//  getPasswordAuthentication( return new PasswordAuthentication("보낸사람이메일", "보내는사람의 이메일 패스워드");)
		
			// session : 인증 후 결과 정보를 담고 있는 객체
		Session session = Session.getDefaultInstance(Properties, authenticator );
				// Session : import javax.mail.Session;
				// .getDefaultInstance ( 호스팅 설정Properties , 인증할 계정정보가 담긴 인증객체 ) 
	 ...
	 - 자바에서 이메일 전송 보내기
		...
		3. 메일 전송 함수 만들기
			...
			1. 호스트 설정
				...
				Properties.put("mail.smtp.host", 호스트포트번호 );
				Properties.put("mail.smtp.auth", true );
				Properties.put("mail.smtp.ssl.protocols", "TLSv1.2");
