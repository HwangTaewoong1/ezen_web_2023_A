...
/*
	...
	- for 문
		...
		1.	인덱스 0부터 배열의 길이만큼 반복
			for( let i = 0 ; i < arrayList.lengh; i++ ){ }
		
		2. 	배열의 첫번째 인덱스부터 마지막 인덱스까지 반복
			for( let 반복변수 in arrayList ){ }
		
		3.	배열의 첫번째 데이터부터 마지막 데이터까지 반복
			for( let 반복변수 of arrayList ){ }
		
		4.  배열의 반복변수 or 인덱스를 첫번째부터 마지막 반복  [ 반환return 값 x ]
			arrayList.forEach( (반복변수)=>{ } )
			arrayList.forEach( (반복변수,인덱스변수)=>{ } )	
			
		5.  배열의 반복변수 or 인덱스를 첫번째부터 마지막 반복	 [ 반환return 값 o ]
			arrayList.map( (반복변수)=>{ } )
			arrayList.map( (반복변수,인덱스변수)=>{ } )	
			let returnArrays = arrayList.map( (반복변수,인데스변수)=>{ return  } );
...
let newArrayList2 = arrayList.forEach( (data5)=>{ return data5 + "map";  } );
