...
public class Ex1_예외처리 {
	...
	public static void printLength(String data) {
		...
		try { // 예외가 발생할 것 같은 코드 [ 경험 의존 , 테스트 ] 
			...
			// NullPointerException : 처음에는 모르지만 .. 실행한 결과 ( 경험을 결과 ) NullPointerException 예외 발생했다는 경험했기 때문에 
			System.out.println(e.getMessage()); // e 객체 : 예외 발생에 관련된 정보 들어있는 객체 [ Cannot invoke "String.length()" because "data" is null ]
			System.out.println( e ); // 실무 : 오류 정보를 파일처리해서 추후에 문제가 발생한 것들을 개발자 업데이트 // 유지보수
			
		}finally { // 예외 발생 하던 안하던 무조건 실행되는 코드 ( 선택사항 )
			System.out.println("마무리 실행");
		}
	} // m e
	
	public static void main(String[] args) {
		// p.466 : 실행 예외 [ - 실행 후 예외가 발생하기 때문에 예외 처리는 경험/테스트 후 예외 처리 ] 
		System.out.println("프로그램 시작");
		printLength("ThisISJava"); // 정상 실행
		printLength(null); // 예외 발생 : NullPointerException => 프로그램 강제 종료 => 아래 코드 실행x
		System.out.println("프로그램 종료 ");
		
		// p.468 : 일반 예외 [ - 실행 전부터 컴파일러가 예외 검사 하기 때문에 예외 필수 !! ] 
		try { // (실행예외) 예외가 발생할 것 같은 코드 + (일반예외) 예외 무조건 발생할것 같은 코드
		...
		try {
			Class.forName("java.lang.String2");	// java.lang.String2 클래스는 존재하지 않으므로 예외 발생!! catch이동 
			System.out.println("java.lang.String 클래스 존재");
		}catch ( ClassNotFoundException e ) { e.printStackTrace(); }
...
/*
 	...
 	예외 : 에러 이외
 		* 프로그램에 예외가 발생했을 때 정상적인 실행 유지할 수 있도록
 		- 1. 일반예외 : 컴파일러가 예외처리 코드 여부를 검사하는 예외
