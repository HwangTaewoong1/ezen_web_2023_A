...
public class Ex1_멀티스레드 {
	public static void main(String[] args) { // 메인스레드가 main메소드를 호출하면서 코드 시작
		...
		// p.596 예1
		Toolkit toolkit = Toolkit.getDefaultToolkit(); // java.awt : JAVAUI
			// * 5번의 비프음 발생 시켰는데.. 비프음 1번 들림 [ 비프음보다 for문이 더 빨라서 ] 
		...
		// -------------------------------- 멀티 스레드 --------------------------------//
		// 1. [main스레드가 작업스레드 생성] main 스레드 외 작업 스레드 객체 만들기 
...
/*
 	프로그램 : 명령문의 집합
 		...
 		* 멀티스레드는 프로그램 *내부에서의 멀티 태스킹
 		...
 		- 멀티스레드에서는 하나의 스레드가 오류가 발생하면 프로세스 종료되므로 영향 미친다.
 			- 카카오톡(프로세스[채팅기능/스레드,첨부파일전송기능/스레드]) 
 	...
 	멀티스레드 : 병렬로 데이터 처리하는 곳
 		...
 		3.	
 		 	...
 		 	멀티스레드: main메소드만 새로운 작업스레드를 생성하는 경우 [ *모든스레드가 종료되면 프로세스 종료 ]
 		4. 작업 스레드 생성
 			1. Thread 클래스로 직접생성
