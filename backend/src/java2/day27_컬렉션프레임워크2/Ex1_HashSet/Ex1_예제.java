...
public class Ex1_예제 {
	...
	public static void main(String[] args) {
		...
		Set<String> set = new HashSet<>();
			// set객체(컬렉션 프레임워크)에 <String>객체를 여러개 저장 가능  
		...
		System.out.println("set 객체내 객체수 : "+ set.size());
		
		// 4. set/list 와 for문 관계 [ * 배열/컬렉션 내 저장된 데이터들을 순회(검색/꺼내/출력) 할려고  ] 
			// 1. for문 [ list는 인덱스가 존재 하므로 가능하지만 , set 불가능.. ]
		...
		// --------------------------------------------------------- //
			...
			// 객체내 중복여부 체크
				...
				// 객체 [ String , int ] 가 2개 이상의 필드를 가지고 있으면 중복기준 명확하지 않기 때문에 
				// hashCode() , equals() 재정의 이용한 중복제거 기준 정하기
		...
		System.out.println("set 객체내 member 객체 수 : " + set2);
		...
		while( iterator.hasNext() ) { // .hasNext() : 다음 객체가 있으면 true / 없으면 false
			Member element = iterator.next(); // .next() : set 객체 내객체 호출
		...
		// 4.향상된 for문 ( iterator 를 사용하는 배열/컬렉션프레임워크의 객체 이면 사용가능)
		for( Member m : set2 ) { System.out.println( m ); }
		
		// 4. forEach( iterator 를 사용하는 컬렉션프레임워크의 객체 이면 사용가능 )
