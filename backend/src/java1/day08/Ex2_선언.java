...
public class Ex2_선언 {
	...
	public static void main(String[] args) {
		...
		Student s1 = new Student();
			// 1. Student : Student 클래스를 가져와서 설계도로 사용
				// 2. s1 : Student 객체의 주소값을 저장할 스택영역의 지역변수
					// 3. new : 객체를 생성하고 생서된 주소값을 스택영역의 지역변수에게 반환
						// 4. Student(); 	: 객체 생성시 초기/처음값을 넣어서 필드에 저장할 수 있는 메소드
		System.out.println(" s1 변수가 Student 객체의 주소를 가지고 있다.");
		// . (도트연산자) : 변수명.메소드() / 변수명.필드명 : 변수가 가지고있는 주소로 이동해서 필드/메소드 사용
		...
		// 3. Student 설계도/클래스 는 같지만 서로 다른 객체가 생성 [ * new연산자는 항상 새로운 주소 할당 ] 
