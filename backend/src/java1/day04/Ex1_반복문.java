...
public class Ex1_반복문 { // class s
	...
	public static void main(String[] args) { // main s
		...
		// [ p.127 ] 실수		// 소수점뒤에 f 사용이유 : { 직접 작성한 } 리터럴 소수점 = *double로 인식하기때문
		...
		System.out.print( 2 * 7 ); System.out.print( 2 * 8 ); System.out.println( 2 * 9 );
			// 2. 2단 구구단 for 이용
		for( int 곱 = 1; 곱<=9; 곱++) {
			System.out.print( 2 * 곱 + "  "); }
			// 3. 2~9단 구구단 for 없이
		// System.out.print( 2 * 1 ); ~ System.out.print( 2 * 9 );
		// System.out.print( 3 * 1 ); ~ System.out.print( 3 * 9 );
		// ~~
		// System.out.print( 9 * 1 ); ~ System.out.print( 9 * 9 );
			// 1. 1 ~ 9단 for 태턴
		for( int 단 = 2; 단<=9; 단++) { System.out.println( 단 ); }
			// 2. 1곱 ~ 9곱 for 패턴
		for( int 곱 = 1; 곱<=9; 곱++) { System.out.println( 곱 ); }
			// 3. 1,2 합치기 [ 단마다 곱 실행(o) vs 곱마다 단 실행 ]
		...
		// [ p. 130 vs 126 ] 1 ~ 100 누적 합계
		
		// [ 126 ]
		// int sum3 = 0;
		// for ( int 반복변수 = 1; 반복변수<=100; 반복변수++ ) { sum3 += 반복변수; } // for문 추천! 1. 가독성이 좋다. 2. 점점 라이브러리 함수에서 for문을 많이 제공한다.
		...
		while ( run ) { // while s
			...
			if ( strNum.equals("1") ) { // 만약에 입력이 1이면
				...
				speed--;	// 스피드 감소
				System.out.println("현재 속도 = " + speed);
			}else if ( strNum.equals("3") ) { // 만약에 입력이 2이면	
				run = false; // 1. // 위치1
				// break; // 2 : 가장 가까운 반복문 탈출 // 위치1
				// return; // 3 : 현재 함수 강제 반환/종료 // 위치2 메인함수 밖으로
			}
		} // while e
		// 위치1
		...
		숫자for : for( int j = 1 ; j<=10; j++) {
			if( j % 2 != 0) { // 홀수 찾기 == 1
				continue 숫자for; // for으로 이동 // 아래에 있는 코드는 실행x // 홀수 출력x
			} // if end
...
/*
 	...
 	for문 
 		...
 		3. true이면 실행문 실행
 		4. 초기값이 들어있는 변수에 증감식
 		5. 조건식 판단해서 ~~~ 2 , 3 , 4 반복 ( 조건식이 false가 될때까지)
