...
public class MemberController { // 기능처리(로직) 담당하는 클래스 // controller -> java
	...
	public String findIdLogic( String name , String phone ) {
		for( int i = 0 ; i< MemberDao.memberList.length; i++ ) {
			if( MemberDao.memberList[i] != null && 						// 1. i번재 인덱스의 값이 null 아니면서 [ 회원이 있는곳만 비교 ]
					MemberDao.memberList[i].getName().equals(name) &&		// 2. i번째 인덱스의 회원객체 의 id가 입력한 id와 같고
						MemberDao.memberList[i].getPhone().equals(phone) ) {	// 3. i번째 인덱스의 회원객체 의 pw가 입력한 pw와 같으면  
				return MemberDao.memberList[i].getId(); // 로그인 성공 
			} // if end  
		} // for end
	...
	public String findPwLogic( String id , String phone) {
		for( int i = 0 ; i< MemberDao.memberList.length; i++ ) {
			if( MemberDao.memberList[i] != null && 						// 1. i번재 인덱스의 값이 null 아니면서 [ 회원이 있는곳만 비교 ]
					MemberDao.memberList[i].getName().equals(id) &&		// 2. i번째 인덱스의 회원객체 의 id가 입력한 id와 같고
						MemberDao.memberList[i].getPhone().equals(phone) ) {	// 3. i번째 인덱스의 회원객체 의 pw가 입력한 pw와 같으면  
				return MemberDao.memberList[i].getPw(); // 로그인 성공 
			} // if end  
		} // for end
