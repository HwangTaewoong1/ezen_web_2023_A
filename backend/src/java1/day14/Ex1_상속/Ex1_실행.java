...
public class Ex1_실행 {
	...
	public static void main(String[] args) {
		...
		// System.out.println( phone.wifi); // 오류 발생 : 자식클래스의 멤버이므로 불가능
		
		phone.bell();
		phone.sendVoice("여보세요");
		phone.receiveVoidce("안녕하세요! 저는 홍길동입니다.");
		phone.hangUp();
		// phone.setWifi( true );		// 오류 발생 : 해당 객체는 부모객체 이고 해당 메소드는 자식 클래스의 멤버 이므로 불가능 
		...
		myPhone.setWifi( true );		// 오류 발생 : 해당 객체는 부모객체 이고 해당 메소드는
		myPhone.internet();
	}
}
