...
public class Ex2_연산자 { // class s // 모든 자바 코드는 클래스에서 작성
	...
	public static void main(String[] args) { // main s // 모든 실행 코드는 main함수에서 
		...
		// 1. 산술 연산자 [ p.81 ] : 연산 후 반환 => 기본자료형 데이터
		...
		// 2. 비교 연산자 [ p.88 ] : 연산 후 반환 => boolean true/false
		System.out.println(" x == y : " + ( x==y ) );		// == 같다  vs JS : 1.== 2.===
		System.out.println(" x != y : " + ( x!=y ) );		// != 같지않다.
		System.out.println(" x > y  : " + ( x > y ) );		// x 는 y 보다 초과	 / y는 x보다 미만
		System.out.println(" x < y  : " + ( x < y ) );		// x 는 y 보다 미만 / y는 x보다 초과
		...
		System.out.println(" x >= 10 or y >= 10 : " + ( x >= 10 || y >= 10 ) );	// 조건이 하나라도 true이면 true / true or flase => true
		System.out.println(" 부정 : " + ( !true ) ); 
		
		// 4. 대입 연산자 [ p.102 ] 
		int a = 30;	// = 대입연산자 : 오른쪽 데이터를 왼쪽 데이터에 대입/저장 [ 기존 데이터 사라짐 ]
		a += x ;	// += : 오른쪽 데이터를 왼쪽 데이터에 더한후에 대입/저장 [ *누적 ]
		...
		String result = ( x >= 20 ) ? "합격" : "불합격";
			System.out.println(result);
						// (조건) ? ( 참 ) : ( 거짓 )
						// (조건) ? ( 참 ) : ( ( 조건 ) ? ( 참 ) : ( 거짓 ) )
		String result2 = ( x>= 10 ) ? "1차합격" : ( y>=10 ) ? "2차합격" : "최종합격";
		
		// 6. 증감 연산자 ++x vs x=x+1 vs x+=1
		System.out.println(" ++x : " + ( ++x )); 	// 해당 명령어에서 x 1증가 선위증가 [ 출력보다 ]
		System.out.println(" x++ : " + ( x++ ));	// 해당 명령어에서 x 1증가 후위증가 [ 출력보다 ]
		System.out.println(" x : " + x);
		
		System.out.println(" --x : " + ( --x ));	// 해당 명령어에서 x 1감소 선위감소 [ 출력보다 ]
		System.out.println(" x-- : " + ( x-- ));	// 해당 명령어에서 x 1감소 후위감소 [ 출력보다 ]
		System.out.println(" x : " + x);
