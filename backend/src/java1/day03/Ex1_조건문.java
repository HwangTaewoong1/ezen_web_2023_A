...
public class Ex1_조건문 { // class s
	...
	public static void main(String[] args) { // main s
		...
		// [ P.115 ] 난수 생성 : Math.random()
		System.out.println( Math.random() ); // 0.0 <= ~ < 1.0
		System.out.println( Math.random()*6 ); // 0.0 <= ~ < 6.0
		System.out.println( (int)(Math.random()*6) ); // 0.0 <= ~ < 6.0
		System.out.println( (int)(Math.random()*6) +1 ); // 1 <= ~ < 7.0
		// 점수 난수공식 : (int) (Math.random() * n ) + s 
			// s: 난수 시작번호			n : 난수 개수
		// 예시 1~45
		...
		// 난수 생성 클래스 : Random 클래스
			...
			// 2. 랜덤 값 호출
				// random.next타입명() : 타입의 허용범위 내 난수생성
		Random random = new Random();
		...
		// 예시) a-z
		System.out.println( (char)(random.nextInt(26) + 97) ); // 97 ~ 122(a~z까지 아스키코드(10진수))
		...
		if ( score3 >= 90 ) { // 점수가 90점 이상이면
			...
			else { grade = "A"; } 	// [ 90 ~ 94 ]이면서 95점 미만이면 A 대입
 	...
 	if : 만약에 조건에 따른 흐름제어
 		형태 
			...
			4. if ( 조건식1 ) { 참1일 경우 실행문 }
				else if( 조건식2 ) { 참2 일경우 실행문 }
				else if( 조건식3 ) { 참3 일경우 실행문 }
				else{ 거짓 일 경우 실행문 }
					* [ 다수 조건에 하나의 실행문 ] 만약에 조건이 충족할 경우 참 실행문 실행 후 전체 if 종료 
			
			5. if ( 조건식1 ) { 참1일 경우 실행문 }
			   if ( 조건식2 ) { 참2일 경우 실행문 }
			   if ( 조건식3 ) { 참3일 경우 실행문 }
			   if ( 조건식4 ) { 참4일 경우 실행문 }
			   		* [ 다수 조건에 다수 실행문 처리 ]
		...
		* switch = 변수 값에 따른 실행
			- 경우의 수의 값이 정해져 있는 경우 / 범위 x
			- break; : switch() /만약에 해당 case 실행 후 멈춤
 */