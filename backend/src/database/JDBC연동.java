...
public class JDBC연동 {
	...
	public static void main(String[] args) {
		...
		// 1. p.908 : 현재 프로젝트에 JDBC 파일을 Bulid path add 하기
		// 2. p.909 : DB연결하기 위해 JDBC Driver 클래스를 찾아서 메모리 로딩하기
		...
		// 3. DB연결 [ DriverManager.getConncetion() 메소드가 연결 성공하면 DB와 연결된 객체 리턴
													// -> 만약에 실패하면 예외발생 -> 예외처리
...
/*
 	예외처리 : 
 		- Exception 클래스
 		- 예외[오류] 발생했을 때 흐름 제어 [ * 만약에 오류가 발생했을때 다른 코드로 흐름을 제어 ]
 		- try{ } catch{ }
 			try{ 예외[오류]가 발생할 것 같은코드 }
