		...
		while( true ) { // 무한루프 [ 종료조건 : 4 입력시 break; ] 
			...
			if( ch == 1 ) { 
				...
				if(account2.equals( account )) {
					...
					System.out.print(">>> 예금후 금액 : " + balance);
				...
				String account3 = scanner.next();
				
				if(account3.equals( account )) {
					System.out.print(">>> 계좌 비밀번호 : " );
					short password2 = scanner.nextShort();
					...
					if(password2 == password ) {
						...
						int balance2 = scanner.nextInt();
							...
							}else { System.out.println(">>> 출금후 금액 : " + (balance-=balance2)); }
					}else { System.out.println(">>> 해당 계좌의 비밀번호가 다릅니다.");
							continue;}
				}else { System.out.print(">>> 없는 계좌번호 입니다.");
							continue;}
			}else if( ch == 3 ) { 
				System.out.println(">>> 예금 잔액 : " + balance );  // 예금액 변수 호출해서 출력 
			}else if( ch == 4 ) { 
