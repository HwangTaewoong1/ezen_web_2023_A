...
public class NoticeBoardDao extends Dao {
	...
	public boolean bwrite(BoardDto boardDto) {
		try {
			String sql = "insert into board( btitle , bcontent , bfile , mno , bcno , mnickname )"
					+ " values( ? ,? , ? ,? , ? , ? )";
			...
			ps.setString(6, boardDto.getMnickname());
	...
	public boolean onUpdate(BoardDto dto) {
		try {
			String sql = "update board " + "	set btitle = ? , bcontent = ? , " + " bcno = ?  , bfile = ? "
					+ "    where bno = ? ";
			ps = conn.prepareStatement(sql);
