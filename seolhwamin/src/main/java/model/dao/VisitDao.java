...
public class VisitDao extends Dao {
	...
	public boolean vwrite( VisitDto visitDto ) {
		// 0. try{} catch(){}
		try {
			String sql = "insert into visitlog( vwriter , vcontent , vimg ) values( ? , ? , ? )"; // 1. SQL 작성  
			ps = conn.prepareStatement(sql); // 2. SQL 연결 
			ps.setString( 1 , visitDto.getVwriter() );  // 3. SQL 매개변수 대입 
			ps.setString( 2 , visitDto.getVcontent() ); 
			ps.setString(3 , visitDto.getVimg());
	...
	public ArrayList<VisitDto> vread() { 
		...
		try {
			...
			while( rs.next() ) { // rs.next() : select 검색결과중 다음레코드 이동 =존재하면 true / 존재하지 않으면 false 
				...
				//VisitDto visitDto = new VisitDto( rs.getInt( "vno" ), rs.getString( "vwriter" ),  rs.getString( "vcontent" ), rs.getString( "vdate" ), rs.getString("vimg") );
	...
	public boolean vupdate( int vno , String vcontent ) { 
		try {
			String sql ="update visitlog set vcontent = ? where vno = ?"; // 1. SQL 작성 
			ps = conn.prepareStatement(sql);	// 2. SQL 연결
			ps.setInt( 2, vno ); ps.setString( 1, vcontent ); // ? 순번 
	...
	public boolean vdelete( int vno ) {
		try {
			String sql ="delete from visitlog where vno = ?"; // 1. SQL 작성 
			ps = conn.prepareStatement(sql);	// 2. SQL 연결
