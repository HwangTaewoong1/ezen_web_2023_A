...
/*
	1. 서버 소켓 만들기
		...
		3. 서버 웹소켓의 필수 메소드
		 	...
		 	@OnMesseage	: 클라이언트소켓이 메시지를 보냈을때.
		 	@OnError	: 클라이언트소켓이 서버소켓과 연결에 에레 발생했을때
...
public class ChattingSocket {
	...
	// 1. 클라이언트가 서버소켓에 접속했을때 매핑/연결 ( JS에서 new WebSocket 객체 생성될때 )
	@OnOpen
	...
	// 2. 클라이언트가 서버소켓과 연결이 닫혔을때 매핑/연결 ( JS에서 웹소켓 객체를 초기화 = 새로고침[F5], 페이지 전환 등등 )
	@OnClose
	...
	public void onMessage(Session session, String msg) {
		...
		// 현재 접속된 명단(클라이언트 소켓)들 에게 메시지 전달
