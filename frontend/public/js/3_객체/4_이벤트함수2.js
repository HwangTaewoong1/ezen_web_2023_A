/*
 	...
 	2.	
 	    ...
 	    2. DOM객체.addEventListener('이벤트명' , ( e )=>{ 실행코드정의 } )				// 익명 화살표 함수 정의해서 적용
 	    3. DOM객체.addEventListener('이벤트명' , function ( e ){ 실행코드 정의 } )		// 익명 함수 정의해서 적용
 	...
 	이벤트
		...
		3. click			: 해당 객체를 클릭 했을 때
		4. keyup			: 해당 객체에서 키보드를 누르고 떼었을 때
		5. keydown			: 해당 객체에서 키보드를 눌렀을 때
			- e.keycode 입력된 키코드번호
 */
// 1.
let text = document.querySelector('.textbox')
text.addEventListener( 'keyup' , ( event )=>{ 		console.log('키보드 입력하고 떼었을 때 ');
	...
	// 만약에 키보드에서 엔터를 쳤을때
	if( event.keyCode == 13 ){ alert('엔터를 입력했습니다.') }
})
...
moving.style.position = `absolute`;	// 2. style 속성 이용한 CSS 추가
	...
	// * 버튼 위치의 초기값 x = 가로축 y = 세로축  block = 이동단뒤[30px씩 이동]
	let x = 7; 	let y = 0; let block = 30;
	// 1. 버튼 위치 출력 함수
movingPrint();	// JS 열렸을 때 최초 1번 실행
