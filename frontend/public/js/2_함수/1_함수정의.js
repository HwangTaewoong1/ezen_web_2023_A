			...
			4. 함수 선언
				...
				function 함수명( 인수1 , 인수2 , 인수3 ){
					...
					**** 코드 정의 ****
					...
					4. { }  	: '{' 함수 시작 / '{' 함수끝 
					5. return 	: 함수 종료/리턴
								return3; 	함수를 호출했던 곳으로
								return; 	반환 값이 없으므로 반환값없이 함수종료
					
					ex) 1. 유재석이 핸드폰 이 고장나서 서비스센터 간다.
...
함수명1();					// 4. JS에서 내가 만든 함수호출
함수명2(3);					// 5. 함수호출과 동시에 해당 함수에 3 전달 [ X = 3 ]
함수명3( 3 , 5 , 20 )			// 6. 함수호출과 동시에 해당함수에게 데이터 3개 전달[ X = 3 , Y = 5 , Z = 20]
