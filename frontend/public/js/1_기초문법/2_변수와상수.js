/*
	...
	- 변수 : 데이터1개 저장하는 메모리공간
		- 저장된 데이터는 변경이 가능하다.
		- let 변수 선언시 사용되는 키워드 [ var 키워드 : 변수명 중복가능 (사용권장 x) ]
		- 변하는 값 , [ 임시 저장 ]
...
// 5. 입력받은 값 저장
let 입력 = prompt('숫자로 입력해주세요.')
console.log('입력받은 숫자 : ' + 입력 )

let 상태 = comfirm('삭제 하시겠습니까?')	// confirm() : 확인(true) or 취소(false) 버튼 존재하는 메시지 함수
