	...
	배열 : 데이터 여러개를 한곳에 저장할 수 있는 공간/메모리/상자 
		...
		7. 배열 내 데이터 추가
				배열명.push( 새로운값 )		: 배열내 마지막 위치에 저장	
					- 마지막 인덱스 뒤에 데이터 추가
				배열명.splice( 위치 인덱스 , 0 , 새로운값 ) : 해당 위치 인덱스에 새로운 값 저장
					-특정 위치/ 인덱스에 데이터 추가
					[* 추가된 요소 뒤로 한칸씩 자동 밀려남 ]
		8. 배열 내 데이터 삭제
				배열명.pop()				: 배열내 마지막 위치 데이터 삭제
				배열명.splice( 제거 할 인덱스 , 제거개수 )
				ex)
		...
		9. 배열 내 데이터를 이용한 데이터 삭제
				- 배열명.indexOf('찾을 데이터')  : 찾을 데이터가 존재하면 찾은인덱스번호 아니면 -1
...
document.write( '</ol>' )
// 예제4 : 인덱스를 이용한 데이터 수정
console.log( 배열명[0] ); 	// 수정 전 '바나나'
배열명[0] = '수박'; 			// 수정
console.log( 배열명[0] );		// 수정 후 '수박'	

// 예제5 : 배열에 요소/데이터 추가
배열명.push( '체리' ); 		//  push('체리') : 배열 내 마지막 요소/ 데이터 뒤에 '체리' 추가
console.log( 배열명 ) 
배열명.splice( 0 , 0 , '복숭아' ) // 0번째 인덱스에서 0개 자르고 '복숭아' 추가 
console.log( 배열명 )
...
배열명.splice( 배열명.indexOf('딸기') , 1 )  //'딸기' 삭제 하는데 인덱스 번호를 모르는상태
	// 1. 배열명.indexOf('딸기') => 2
