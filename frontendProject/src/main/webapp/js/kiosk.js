console.log('kiosk.js 실행')

// 1. 광고 이미지 변경
	// 1. 광고 이미지 파일명을  여러개 저장하고 있는 공간 = 배열
...
categoryPrint( 0 ); // 최초 1번 실행 [가장 앞에있는 카테고리 선택 가정]
function categoryPrint( selectNo ){
	// 1. 어디에
	let categorymenu = document.querySelector('.categorymenu');
	// 2. 무엇을
	let html = ``;
		// HTML 구성 : 배열 내 모든 데이터를(for) li 형식으로 출력 
	for( let i = 0; i<categoryList.length; i++){
		// 선택된 카테고리(selectNo) 와 i 와 같으면 i번째 인덱스 선택됨 카테고리
...
// 3.카테고리 클릭 함수 { 실행조건 : 1. li에서 클릭했을 때 }
function categorySelect( selectNo ){
	// ?? <li> 여러개 존재 하는데 무엇 선택했는지 식별 
	console.log(selectNo);
	
	// 0. 카테고리의 모든 li 호출 [ document.querySelector 1개 호출 vs document.querySelectorAll 여러개 호출 = 여러개객체(배열) ]
	...
	for ( let i = 0; i<categoryList.length; i++){
		...
		if( i == selectNo){
			...
			// 해당 li 에 class 삭제 DOM객체명.classList.remove('삭제할 클래스명')
